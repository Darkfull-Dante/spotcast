    ...
    async def start_session(self):
        ...
        async with aiohttp.ClientSession(cookies=cookies) as session:
            ...
            async with session.get('https://open.spotify.com/get_access_token?reason=transport&productType=web_player', allow_redirects=False, headers=headers) as response:
                ...
                if not response.ok:
                    _LOGGER.info("Unsuccessful token request, got response code %i", response.status)
                    raise TokenError(f"Unsuccessful token request, got response code {response.status}")
